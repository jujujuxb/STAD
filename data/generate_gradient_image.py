...
src_path = '/home/juxiaobing/code/GraduationProject/CNN-VAE/data/T15/anotation_txt'
dest_path = '/home /juxiaobing/code/GraduationProject/CNN-VAE/data/T15/anatation_images'
...
for fld in classes:   # assuming data directory has a separate folder for each class, and that each folder is named after the class
    ...
    # specify the kind of files if * is used all will be added
    path = os.path.join(src_path, fld, '*.txt')
    # path = os.path.join(src_path, '*.txt')

    print(path)

    # print path
    files = glob.glob(path)
    for fl in track(files, description="load image:"):
       # print(classes[index])
       # print(fl)
        ...
        file_name = fl.split('/')[-1].replace('txt', 'jpg')
        desedirfile = os.path.join(dest_path, fld)
        destfile = os.path.join(desedirfile, file_name)
        if not os.path.exists(desedirfile):
            os.makedirs(desedirfile)
        print(dest_path+fld)
        print(destfile)
        ...
        y = []
        t = []
        # comment this if you want to see all trajectory
        hsv_image[:, :, :] = 0
        # print(hsv_image)
